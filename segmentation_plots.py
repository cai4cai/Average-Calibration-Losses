...
from sacros.transforms import ConvertToKits23Classesd, ConvertToKits23ClassesSoftmaxd
from sacros.brats_transforms import ConvertToBratsClassesd, ConvertToBratsClassesSoftmaxd
...
class Model(ABC):
    def __init__(self, data_paths, transforms, post_transforms, in_channels=1, out_channels=4, ckpt_path=None, roi_size=(128, 128, 128), case_name_batch_transform=None):
        """
        ...
        self.inferer = SlidingWindowInferer(roi_size=self.roi_size, overlap=0.25, sw_batch_size=1, mode="gaussian", sigma_scale=0.125)
...
class KiTSModel(Model):
    def __init__(self, loss="soft", version="nl", hec=True):
        ...
        self.num_classes = 4
        if hec:
            self.class_names = ['background', 'tumour', 'kidney_mass', 'kindey_and_masses']
        else:
        ...
        case_name_batch_transform = lambda b: b['image'].meta[Key.FILENAME_OR_OBJ].split("/")[-2]
        ...
        super().__init__(data_paths, transforms, post_transforms, in_channels, out_channels, ckpt_path, roi_size, case_name_batch_transform)
    ...
    def make_data_paths(self, dataset_dir, test_seed):
        ...
        partitions_train_test = partition_dataset(all_dicts, (0.8, 0.2), shuffle=True, seed=test_seed)
...
class BraTSModel(Model):
    def __init__(self, loss="soft", version="nl", hec=True):
        ...
        self.num_classes = out_channels
        if hec:
            self.class_names = ['Background (BG)', 'Enhancing Tumour (ET)', 'Tumour Core (TC)', 'Whole Tumour (WT)']  # HEC classes
        else:
            self.class_names = ['Background (BG)', 'Necrotic Tumour Core (NCR)', 'Peritumoral Edema (ED)', 'Enhancing Tumour (ET)']  # original classes
        ...
        case_name_batch_transform = lambda b: "_".join(b['image'].meta[Key.FILENAME_OR_OBJ].split("/")[-1].split("_")[:2])
    ...
    def make_data_paths(self, dataset_dir):
        ...
        imgs = sorted(glob.glob(os.path.join(dataset_dir, 'BraTS2021_*', 'BraTS2021_*_t1.nii.gz')))
        data_dicts = []
        for i in imgs:
            data_dicts.append({
                "t1": i,
    ...
    def make_transforms(self):
        ...
        return Compose([
            LoadImaged(keys=["t1", "t1ce", "t2", "flair", "label"], image_only=True),
...
class AMOSModel(Model):
    def __init__(self, loss="soft", version="nl"):
        ...
        out_channels = 16
        self.num_classes = out_channels
        self.class_names = ['background', 'spleen', 'right_kidney', 'left_kidney', 'gallbladder', 'esophagus', 'liver', 'stomach', 'aorta', 'inferior_vena_cava', 'pancreas', 'right_adrenal_gland', 'left_adrenal_gland', 'duodenum', 'bladder', 'prostate or uterus']
        roi_size = (192, 192, 64)
        dataset_dir = '../data/amos22'
        case_name_batch_transform = lambda b: "_".join(os.path.basename(b['image'].meta[Key.FILENAME_OR_OBJ]).split('.')[0].split("_")[:2])
    ...
    def make_data_paths(self, dataset_dir):
        ...
        all_images = sorted([os.path.join(dataset_dir, "imagesVa", f) for f in os.listdir(os.path.join(dataset_dir, "imagesVa"))
                             if f.startswith("amos_") and int(f.split("_")[1].split(".")[0]) < 500])
        all_labels = [os.path.join(dataset_dir, "labelsVa", os.path.basename(img)) for img in all_images]
        return [{'image': img, 'label': lbl} for img, lbl in zip(all_images, all_labels)]
    ...
    def make_transforms(self):
        return Compose([
            ...
            CropForegroundd(keys=["image", "label"], source_key="image"),
            Orientationd(keys=["image", "label"], axcodes="RAS"),
            Spacingd(keys=["image", "label"], pixdim=(1.0, 1.0, 5.0), mode=("bilinear", "nearest")),
...
class ACDCSModel(Model):
    def __init__(self, loss="soft", version="1"):
        ...
        out_channels = 4
        self.num_classes = out_channels
        self.class_names = ['background', 'right_ventricle', 'myocardium', 'left_ventricle']
        ...
        for patient in os.listdir(base_dir):
            ...
            if os.path.isdir(patient_path):
                for frame in os.listdir(patient_path):
                    if frame.endswith('.nii.gz') and 'frame' in frame and '_gt' not in frame:
                        image_path = os.path.join(patient_path, frame)
                        label_path = os.path.join(patient_path, frame.replace('.nii.gz', '_gt.nii.gz'))
...
def extract_component(
    arrays, component=3, keys_to_extract=None, exclude_keys=['image', Key.FILENAME_OR_OBJ]
):
...
def make_plot(baseline_out, 
    ...
    fig, axs = plt.subplots(
        1, 5, figsize=(15, 5), gridspec_kw={"width_ratios": [1, 1, 1, 1, 0.05]}  # including colorbar
    )
    ...
    plt.savefig(os.path.join(out_dir, f"{baseline_out[Key.FILENAME_OR_OBJ]}_{class_name}.pdf"), bbox_inches="tight")
    plt.close(fig)
...
def make_acdc_plot(acdc_case, baseline_model, hard_model, soft_model, min_size=50):
    ...
    for component_idx in range(1, baseline_model.num_classes):
        baseline_out_comp = extract_component(baseline_out.copy(), component=component_idx)
        ...
        roi = find_roi_slice(baseline_out_comp, key="label", padding=border_size_2d, square=True)
        ...
        make_plot(baseline_out_comp, hard_out_comp, soft_out_comp, out_dir="./seg_plots/seg_plots_acdc_all", class_name=class_name, min_size=min_size)


def make_acdc_plot_select(case_class_dict, baseline_model, hard_model, soft_model, min_size=50):
    """
    ...
    for class_idx, case_list in case_class_dict.items():
        for case_idx in case_list:
            ...
            baseline_out_comp = extract_component(baseline_out.copy(), component=class_idx)
            baseline_out_comp['image'] = baseline_out_comp['image'].squeeze(0)
            ...
            make_plot(baseline_out_comp, hard_out_comp, soft_out_comp,
                      out_dir="./seg_plots/seg_plots_acdc_select", class_name=class_name, min_size=min_size)
...
def make_amos_plot(amos_case, baseline_model, hard_model, soft_model, min_size=50):
    ...
    for component_idx in range(1, baseline_model.num_classes):
        ...
        class_name = baseline_model.class_names[component_idx]
        make_plot(baseline_out_comp, hard_out_comp, soft_out_comp, out_dir="./seg_plots/seg_plots_amos_all", class_name=class_name, min_size=min_size)
        

def make_amos_plot_select(case_class_dict, baseline_model, hard_model, soft_model, min_size=50):
    border_size_2d = (10, 10, 10, 10)
    for class_idx, case_list in case_class_dict.items():
        for case_idx in case_list:
            baseline_out = baseline_model.run_inference_case(case_idx)
            ...
            class_name = baseline_model.class_names[class_idx]
            make_plot(baseline_out_comp, hard_out_comp, soft_out_comp,
                      out_dir="./seg_plots/seg_plots_amos_select", class_name=class_name, min_size=min_size)


def make_brats_plot(brats_case, baseline_model, hard_model, soft_model, hec = True, min_size=50):
    border_size_2d = (10, 10, 10, 10)
    ...
    for component_idx in range(1, baseline_model.num_classes):
        ...
        make_plot(baseline_out_comp, hard_out_comp, soft_out_comp, out_dir=out_dir, class_name=class_name, min_size=min_size)
        
        
def make_brats_plot_select(case_class_dict, baseline_model, hard_model, soft_model, hec=True, min_size=50):
    """
    ...
    for class_idx, case_list in case_class_dict.items():
        for case_idx in case_list:
            ...
            class_name = baseline_model.class_names[class_idx]
            out_dir = "./seg_plots/seg_plots_brats_hec_select" if hec else "./seg_plots_brats_no_hec_select"

            make_plot(baseline_out_comp, hard_out_comp, soft_out_comp,
                      out_dir=out_dir, class_name=class_name, min_size=min_size)
        

def make_kits_plot(kits_case, baseline_model, hard_model, soft_model, hec=True, min_size=20):
    border_size_2d = (10, 10, 10, 10)
...
def make_kits_plot_select(case_class_dict, baseline_model, hard_model, soft_model, hec=True, min_size=50):
    """
...
if __name__ == "__main__":
    ...
    if args.dataset == "acdc":
        ...
        if args.mode == "all":
            ...
            make_acdc_plot_select(idxs, acdc_baseline, acdc_hard, acdc_soft, min_size=20)
        ...
        if args.mode == "all":
            ...
            idxs = {1: [74, 56, 49], 2: [49, 18, 6], 3: [27, 16, 92], 4: [19, 67, 86], 5: [65, 97, 71], 6: [25, 85, 88], 7: [87, 94, 85], 8: [31, 37, 55], 9: [30, 56, 54], 10: [94, 86, 1], 11: [73, 50, 98], 12: [26, 35, 66], 13: [63, 87, 33], 14: [44, 6, 37], 15: [18, 20, 44]}
        ...
        if args.mode == "all":
            for case in range(len(brats_baseline.dataset)):  # all cases
                make_brats_plot(case, brats_baseline, brats_hard, brats_soft, args.hec, min_size=5)
        elif args.mode == "select":
            if args.hec:
                idxs = {1: [176, 172, 179], 2: [176, 179, 113], 3: [86, 113, 160]}
            else:
                idxs = {1: [172, 195, 176], 2: [113, 86, 109], 3: [176, 172, 179]}
            make_brats_plot_select(idxs, brats_baseline, brats_hard, brats_soft, args.hec, min_size=5)
        ...
        if args.mode == "all":
            for case in range(len(kits_soft.data_paths)):
                make_kits_plot(case, kits_baseline, kits_hard, kits_soft, hec=args.hec, min_size=20)
        elif args.mode == "select":
            if args.hec:
                idxs = {1: [90, 31, 95], 2: [31, 90, 19], 3: [19, 47, 31]}
            else:
                idxs = {1: [23, 47, 85], 2: [90, 31, 95], 3: [93, 78, 90]}
            make_kits_plot_select(idxs, kits_baseline, kits_hard, kits_soft, hec=args.hec, min_size=20)
